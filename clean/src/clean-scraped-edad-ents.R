...
for (i in 1:length(input_file_names)){
  ...
  # Extract ent_code (between "/ent_" and "/estatus_victimas")
  ent_code <- str_extract(file_name, "(?<=ent_)(.*)(?=/estatus_victimas)") %>% 
    as.numeric()
...
ents_edad_10_19_by_year <- bind_rows(tempo_ls) %>% 
...
stopifnot(all(!is.na(ents_edad_10_19_by_year$entidad)),
          all(c("no_local", "local_sv", "local_cv") %in% ents_edad_10_19_by_year$status))

# EXPORT ====
write_csv(ents_edad_10_19_by_year, paste0(tempdir(), "/ents_edad_10_19_by_year.csv"))

drive_upload(paste0(tempdir(), "/ents_edad_10_19_by_year.csv"),
             motha_drive_id,
             overwrite = TRUE)

file.remove(paste0(tempdir(), "/ents_edad_10_19_by_year.csv"))
